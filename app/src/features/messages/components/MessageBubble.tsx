...
export function MessageBubble({
  ...
  return (
    ...
    >
      {!own && !grouped ? (
        <SafeImage className="message-bubble__avatar" src={message.senderAvatar} alt="" />
      ) : null}
      <div className="message-bubble__body">
        {!own && !grouped ? <strong>{message.senderName || "Deleted player"}</strong> : null}
        {message.isAnnouncement ? (
          <span className="message-bubble__announcement">Announcement</span>
        ) : null}
        {!pending ? (
          <div className="message-bubble__actions">
            <button
              ...
              aria-label={`Reply to ${own ? "your message" : message.senderName || "message"}`}
              title="Reply"
            >
              <Reply size={14} />
            </button>
            {own ? (
              <div ref={menuRef}>
                ...
                {menuOpen ? (
                  <div className="message-bubble__menu">
                    <button
                      type="button"
                      onClick={() => {
                        setMenuOpen(false);
        ...
        <footer>
          <time dateTime={message.timestamp ?? message.createdAt}>
            {messageTime(message.timestamp ?? message.createdAt)}
          </time>
          {own && !pending ? (
            <span>
              <Check size={12} />
