...
function MessageHistorySkeleton() {
  return (
    <div className="message-history-skeleton" aria-label="Loading messages" role="status">
      {Array.from({ length: 5 }, (_, index) => (
        <div key={index} data-own={index % 3 === 1 || undefined}>
          {index % 3 !== 1 ? <SkeletonAvatar size={28} /> : null}
          <span>
            <Skeleton width={index % 2 ? 210 : 270} height={58} />
...
function sameGroup(message: ChatMessage, previous?: ChatMessage) {
  if (
    ...
    previous.isAnnouncement !== message.isAnnouncement
  )
    return false;
...
export function MessageThread({
  ...
  const teamQuery = useMessageTeam(
    conversation.type === "user" ? undefined : conversation.team?.id,
  );
  ...
  const loadOlder = async () => {
    ...
    requestAnimationFrame(() => {
      node.scrollTop = oldTop + node.scrollHeight - oldHeight;
    });
  };
  ...
  const normalizedRole =
    `${currentMembership?.role ?? ""} ${currentMembership?.title ?? ""}`.toLowerCase();
  const canAnnounce =
    conversation.type === "team" &&
    ["owner", "leader", "vice leader", "deputy leader", "manager"].some((role) =>
  ...
  return (
    <section className="message-thread" aria-label={`Conversation with ${identity.name}`}>
      <header>
        <button
          ...
          <p>
            {conversation.type === "user"
              ? identity.teammate?.gamerTitle ||
                (identity.online ? "Online" : "Direct conversation")
              : conversation.type === "team-inbox"
                ? "Team inbox"
                : `${conversation.team?.members?.length ?? conversation.participants.length} members`}
          </p>
        </div>
        {status !== "online" ? (
          <span className="message-thread__status" data-status={status} role="status">
            <i />
            {status === "connecting" ? "Reconnecting messages" : "Messages offline"}
          </span>
        ) : null}
      </header>
      <div className="message-history" ref={scrollRef}>
        {hasOlder && !loading ? (
          <button
            className="message-history__older"
            type="button"
            onClick={() => void loadOlder()}
            disabled={fetchingOlder}
          >
            {fetchingOlder ? "Loading earlier messages…" : "Load earlier messages"}
          </button>
        ) : null}
        {loading ? <MessageHistorySkeleton /> : null}
        {error ? (
          <div className="message-history__state">
            <MessageCircle size={20} />
            <strong>Messages could not load</strong>
            <p>The conversation is still here. Try loading it again.</p>
        ...
        {!loading && !error && !messages.length ? (
          <div className="message-history__state">
            ...
            <p>Send a clear first message—no ceremony needed.</p>
          </div>
        ) : null}
        {!loading && !error
          ? messages.map((message, index) => (
              <MessageBubble
                ...
                grouped={sameGroup(message, messages[index - 1])}
                onReply={startReply}
                onEdit={startEdit}
      ...
      {deleteTarget ? (
        ...
        >
          <p>
            This message will be removed from the conversation for everyone. This cannot be undone.
          </p>
          {remove.isError ? (
            <p className="message-inline-error" role="alert">
              {getApiErrorMessage(remove.error, "The message could not be deleted.")}
            </p>
          ) : null}
          <footer>
            <button type="button" onClick={() => setDeleteTarget(null)} disabled={remove.isPending}>
              Keep message
            </button>
            <button
              className="is-danger"
