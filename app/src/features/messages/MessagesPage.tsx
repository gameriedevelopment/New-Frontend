...
export function MessagesPage() {
  ...
  const selected = conversations.find((conversation) => conversation.id === selectedId) ?? flattenConversations(conversationsQuery.data).find((conversation) => conversation.id === selectedId) ?? null;
  ...
  return <div className="messages-page" data-thread-open={Boolean(selectedId) || undefined} style={viewport ? { "--messages-viewport-height": `${viewport.height}px`, "--messages-viewport-top": `${viewport.top}px` } as React.CSSProperties : undefined}>
    <aside className="messages-sidebar">
      <header><div><p>Communication</p><h1>Messages</h1></div></header>
