...
export function MessagesPage() {
  ...
  const conversations = useMemo(() => {
    const source =
      debouncedSearch.length >= 2
        ? (searchQuery.data ?? [])
        : flattenConversations(conversationsQuery.data);
    return source.filter((conversation) =>
      type === "user" ? conversation.type === "user" : conversation.type !== "user",
    );
  }, [conversationsQuery.data, debouncedSearch, searchQuery.data, type]);
  const selected =
    conversations.find((conversation) => conversation.id === selectedId) ??
    flattenConversations(conversationsQuery.data).find(
      (conversation) => conversation.id === selectedId,
    ) ??
    (createdConversation?.id === selectedId ? createdConversation : null);
  ...
  useEffect(() => {
    if (selectedId || !conversations.length || window.matchMedia("(max-width: 760px)").matches)
      return;
  ...
  const switchType = (next: "user" | "team") => {
    setSearch("");
    setParams({ tab: next }, { replace: true });
  };
  const created = (conversation: MessageConversation, tab: "user" | "team") => {
    setCreatedConversation(conversation);
    setNewConversationOpen(false);
    setSearch("");
    setParams({ tab, conversation: conversation.id }, { replace: true });
  };

  return (
    <div
      className="messages-page"
      data-thread-open={Boolean(selectedId) || undefined}
      style={
        viewport
          ? ({
              "--messages-viewport-height": `${viewport.height}px`,
              "--messages-viewport-top": `${viewport.top}px`,
    ...
    >
      <aside className="messages-sidebar">
        ...
        <nav aria-label="Conversation type">
          <button
            type="button"
          ...
          <button
            ...
            onClick={() => switchType("team")}
            aria-pressed={type === "team"}
          >
            Teams
          </button>
        </nav>
        {conversationsQuery.isError && !debouncedSearch ? (
          <StatePanel
            tone="error"
            icon={<AlertCircle size={18} />}
            title="Conversations could not load"
            description="Gamerie could not reach your messages right now."
            action={
              <Button size="small" variant="quiet" onClick={() => conversationsQuery.refetch()}>
                <RefreshCw size={14} />
                Retry
              </Button>
            }
          ...
          <ConversationList
            ...
            loading={conversationsQuery.isLoading || searchQuery.isFetching}
            search={search}
            onSearch={setSearch}
      ...
      <section className="messages-workspace">
        {selected ? (
          <MessageThread
            ...
            messages={messages}
            unread={Number(unreadQuery.data ?? selected.unreadCount ?? 0)}
            loading={messagesQuery.isLoading}
            error={messagesQuery.isError}
            fetchingOlder={messagesQuery.isFetchingNextPage}
            hasOlder={Boolean(messagesQuery.hasNextPage)}
            onLoadOlder={() => messagesQuery.fetchNextPage()}
            onRetry={() => messagesQuery.refetch()}
            onBack={() => setParams({ tab: type }, { replace: true })}
          />
        ) : (
          <div className="messages-empty">
            ...
            <p>Choose a player or team from the list to continue where you left off.</p>
          </div>
        )}
      </section>
      {newConversationOpen ? (
        <NewConversationDialog onClose={() => setNewConversationOpen(false)} onCreated={created} />
      ) : null}
    </div>
  );
}
