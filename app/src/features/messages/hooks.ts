...
import { connectMessagingSocket, createConversation, deleteMessage, editMessage, getConversationUnread, getConversations, getMessages, getTeamDetails, getTotalUnread, getUserTeams, markConversationRead, searchConversations, searchTeams, searchUsers, sendMessage } from "./api";
import type { ChatMessage, ConversationsPage, CreateConversationPayload, MessageConversation, MessagesPage, RecipientTeam, RecipientUser } from "./types";
...
function removeMessage(data: InfiniteData<MessagesPage> | undefined, messageId: string) {
  if (!data) return data;
  return { ...data, pages: data.pages.map((page) => ({ ...page, data: page.data.filter((message) => message.id !== messageId) })) };
}
...
export function useEditMessage(conversationId: string) {
  ...
  return useMutation({
    ...
    onMutate: async ({ messageId, content }) => {
      await client.cancelQueries({ queryKey: key });
      const snapshot = client.getQueryData<InfiniteData<MessagesPage>>(key);
      client.setQueryData(key, (data: InfiniteData<MessagesPage> | undefined) => data ? { ...data, pages: data.pages.map((page) => ({ ...page, data: page.data.map((message) => message.id === messageId ? { ...message, content } : message) })) } : data);
      return { snapshot };
    },
    onSuccess: (message) => client.setQueryData(key, (data: InfiniteData<MessagesPage> | undefined) => replaceMessage(data, message.id, message)),
    onError: (_error, _variables, context) => client.setQueryData(key, context?.snapshot),
    onSettled: () => { client.invalidateQueries({ queryKey: key }); client.invalidateQueries({ queryKey: ["conversations"] }); },
  });
}
...
export function useDeleteMessage(conversationId: string) {
  ...
  return useMutation({
    ...
    onMutate: async (messageId) => {
      await client.cancelQueries({ queryKey: key });
      const snapshot = client.getQueryData<InfiniteData<MessagesPage>>(key);
      client.setQueryData(key, (data: InfiniteData<MessagesPage> | undefined) => removeMessage(data, messageId));
      return { snapshot };
    },
...
export function useRecipientSearch(type: "user" | "team", search: string) {
  return useQuery<Array<RecipientUser | RecipientTeam>>({ queryKey: ["message-recipients", type, search], queryFn: async () => type === "user" ? await searchUsers(search) : await searchTeams(search), enabled: search.trim().length >= 2, staleTime: 30_000 });
}

export function useUserTeams(userId?: string) {
  return useQuery({ queryKey: ["user-teams", userId], queryFn: () => getUserTeams(userId!), enabled: Boolean(userId), staleTime: 5 * 60_000 });
}

export function useMessageTeam(teamId?: string) {
  return useQuery({ queryKey: ["message-team", teamId], queryFn: () => getTeamDetails(teamId!), enabled: Boolean(teamId), staleTime: 5 * 60_000 });
}
