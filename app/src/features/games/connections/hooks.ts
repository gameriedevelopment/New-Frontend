...
export function useGameConnections(enabled = true) {
  return useQuery({
    queryKey: ["game-connections"],
    queryFn: getGameConnections,
    enabled,
    staleTime: 60_000,
  });
}
export function useConnectProvider() {
  ...
  return useMutation({
    ...
    onSuccess: () => client.invalidateQueries({ queryKey: ["game-connections"] }),
  });
}
...
export function useProviderMetrics(connection?: GameConnection, enabled = true) {
  return useQuery({
    queryKey: ["game-connections", connection?.provider, connection?.providerAccountId, "stats"],
    queryFn: () => getProviderMetrics(connection!),
