...
export async function connectProvider(
  ...
  if (provider === "lichess") {
    const { data } = await api.post<ApiEnvelope<GameConnection>>(
      `/game-connections/${provider}`,
      payload,
    );
    return data.data;
  }
  const endpoint =
    provider === "league-of-legends" || provider === "valorant" || provider === "teamfight-tactics"
      ? `/riot/${riotProduct(provider)}/connect`
      : provider === "steam"
        ? "/steam-connect/link"
        : `/${provider}/connect`;
...
const metrics = (source: unknown, fields: Array<[string, string]>): ProviderMetric[] =>
  fields
    .map(([label, path]) => ({ label, value: display(read(source, path)) }))
...
export async function getProviderMetrics(connection: GameConnection): Promise<ProviderMetric[]> {
  ...
  if (connection.provider === "steam")
    return metrics({ connection }, [
      ["Account", "connection.handle"],
      ["Steam profile", "connection.metadata.profileUrl"],
    ]);
  if (connection.provider === "lichess") {
    const response = await fetch(`https://lichess.org/api/user/${handle}`, {
  ...
  if (["clash-royale", "clash-of-clans", "brawl-stars"].includes(connection.provider)) {
    ...
    return connection.provider === "clash-of-clans"
      ? metrics(data, [
          ["Trophies", "trophies"],
      ...
      : connection.provider === "brawl-stars"
        ? metrics(data, [
            ["Trophies", "trophies"],
            ["Best", "highestTrophies"],
            ["3v3 wins", "3vs3Victories"],
  ...
  if (connection.provider === "dota-2" || connection.provider === "cs2") {
    ...
    return connection.provider === "cs2"
      ? metrics(data, [
          ["Wins", "summary.wins"],
          ["Kills", "summary.kills"],
          ["Headshots", "summary.headshots"],
          ["Achievements", "summary.achievementsUnlocked"],
        ])
      : metrics(data, [
          ["Wins", "summary.wins"],
          ["Losses", "summary.losses"],
          ["Average kills", "summary.averageKills"],
          ["Recent matches", "recentMatchCount"],
        ]);
  }
  if (connection.provider === "pubg") {
    ...
    return metrics(data, [
      ...
      ["Recent matches", "latestMatches.length"],
    ]);
  }
  if (connection.provider === "battlenet") {
    const region = encodeURIComponent(String(metadata.region || "us"));
    const realm = encodeURIComponent(String(metadata.realmId || "1"));
    const profile = encodeURIComponent(String(metadata.profileId || connection.providerAccountId));
    const { data } = await api.get(`/battlenet/sc2/profile/${region}/${realm}/${profile}`);
    return metrics(data, [
      ["Player", "summary.displayName"],
      ["Clan", "summary.clanName"],
      ["Achievement points", "summary.achievementPoints"],
      ["Career games", "career.totalGames"],
    ]);
  }
  ...
  if (connection.provider === "league-of-legends") {
    const { data } = await api.get(`/riot/lol/ranked/${id}`, { params: { region } });
  ...
  if (connection.provider === "valorant") {
    ...
    return metrics(data, [
      ...
      ["Rating", "current_data.ranking_in_tier"],
      ["Elo", "current_data.elo"],
      ["Level", "account_level"],
    ]);
  }
  const { data } = await api.get(
    `/riot/tft/summoner/${encodeURIComponent(gameName)}/${encodeURIComponent(tagLine || "")}`,
    { params: { region } },
  );
  return metrics(data, [
...
export async function startRedirectConnection(
  provider: "steam" | "dota-2" | "cs2" | "lichess",
  returnTo: string,
) {
  const state = base64Url(crypto.getRandomValues(new Uint8Array(24)));
  sessionStorage.setItem(CONNECTION_STATE, state);
  sessionStorage.setItem(CONNECTION_PROVIDER, provider);
  sessionStorage.setItem(CONNECTION_RETURN, returnTo);
  if (provider === "lichess") {
  ...
  url.searchParams.set("returnTo", `${window.location.origin}/steam/connect/callback`);
...
export function readConnectionCallback() {
  ...
  const returnTo =
    storedReturn?.startsWith("/") && !storedReturn.startsWith("//") ? storedReturn : "/feed";
  return {
    state: sessionStorage.getItem(CONNECTION_STATE),
    provider: sessionStorage.getItem(CONNECTION_PROVIDER) as GameProviderId | null,
    returnTo,
    verifier: sessionStorage.getItem(LICHESS_VERIFIER),
  };
}
export function clearConnectionCallback() {
  [CONNECTION_STATE, CONNECTION_PROVIDER, CONNECTION_RETURN, LICHESS_VERIFIER].forEach((key) =>
    sessionStorage.removeItem(key),
  );
}

export async function finishLichessConnection(
  code: string,
  verifier: string,
): Promise<GameConnection> {
  const redirectUri = `${window.location.origin}/lichess/connect/callback`;
  ...
  return connectProvider("lichess", {
