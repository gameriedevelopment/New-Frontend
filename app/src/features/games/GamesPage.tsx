...
export function GamesPage() {
  ...
  const [filtersOpen, setFiltersOpen] = useState(() =>
    ["type", "platform", "followers"].some((key) => params.has(key)),
  );
  ...
  const filters = useMemo<GameFilters>(
    () => ({
      ...
      gameType: params.get("type") || undefined,
      platform: params.get("platform") || undefined,
      minFollowers: params.get("followers") || undefined,
    }),
    [params],
  );
  ...
  const dynamicPlatforms = useMemo(
    () =>
      Array.from(
        new Set([...knownPlatforms, ...facetGames.flatMap((game) => game.platforms ?? [])]),
      ).sort(),
    [facetGames],
  );
  const activeFilters = [filters.gameType, filters.platform, filters.minFollowers].filter(
    Boolean,
  ).length;
  const clear = () => {
    setSearch("");
  ...
  return (
    <main className="games-page">
      ...
      <section className="games-toolbar" aria-label="Game catalogue filters">
        <div className="games-search">
          ...
          <label>
            ...
            <input
              ...
              placeholder="Search titles, studios, genres, or descriptions"
              autoComplete="off"
            />
          </label>
          ...
          <button
            ...
            aria-controls="game-catalogue-filters"
        ...
        <div className="games-filters" id="game-catalogue-filters" hidden={!filtersOpen}>
          <label>
            <span>Type</span>
            <select
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => change("followers", event.target.value || undefined)}
            >
      ...
      {query.isLoading ? (
        ...
        <StatePanel
          tone="error"
          title="The games directory could not load"
          description={getApiErrorMessage(
            query.error,
            "Gamerie could not retrieve the catalogue right now.",
          )}
          action={
        ...
        <>
          <div className="game-grid">
            {games.map((game) => (
              <GameCard key={game.id} game={game} stats={stats.data?.[game.id]} />
            ))}
          </div>
          <InfiniteGameLoad
        ...
        <StatePanel
          ...
          description="Try a broader title or clear one of the active filters."
          action={
