...
import { AchievementsPanel, GamesAndRankings, MatchHistory, ProfilePosts, SalaryInsights, SkillsPanel } from "./ProfileDataSections";
...
export function ProfileContent({ own, profile, tab }: { own: boolean; profile: PlayerProfile; tab: ProfileTab }) {
  ...
  if (tab === "career") {
    const teams = (profile.teams ?? []).map((item) => ({ id: item.id, title: item.team?.name || item.name || "Team", detail: item.title || item.role || "Member" }));
    return <div className="profile-career-layout">{teams.length ? <section className="profile-team-history"><header><p>Team history</p><h2>Teams and roles</h2></header><div>{teams.map((item, index) => <article key={item.id || index}><span><Briefcase size={15} /></span><div><h3>{item.title}</h3><p>{item.detail}</p></div></article>)}</div></section> : null}<MilestonesPanel own={own} profile={profile} />{own ? <SalaryInsights profile={profile} /> : null}</div>;
  }
