...
export function ProfileHeader({ own, profile }: { own: boolean; profile: PlayerProfile }) {
  ...
  const follow = useTogglePlayerFollow(
    currentUser?.id,
    profile.id,
  ...
  const message = async () => {
    ...
    const conversation = await createConversation.mutateAsync({
      participantIds: [currentUser.id, profile.id],
      type: "user",
    ...
    <>
      <section className="profile-hero">
            ...
            {own ? (
              ...
              <>
                ...
                <Button
                  ...
                  onClick={() =>
                    navigate(
                      `/challenges?compose=user&target=${encodeURIComponent(profile.id)}&targetName=${encodeURIComponent(profile.username)}`,
                    )
                  }
                >
                  <Swords size={16} />
        ...
        <div className="profile-hero__metrics">
          <button
            type="button"
            aria-label={`${followers.toLocaleString()} followers. Open followers list`}
            aria-haspopup="dialog"
            aria-expanded={network === "followers"}
            onClick={() => setNetwork("followers")}
          >
            <span>Followers</span>
          ...
          <button
            type="button"
            aria-label={`${following.toLocaleString()} following. Open following list`}
            aria-haspopup="dialog"
            aria-expanded={network === "following"}
          ...
          <div>
            ...
            <strong>{profile.gameLevel || "Not set"}</strong>
          </div>
          <div>
            <span>Platforms</span>
            <strong>{profile.platforms?.length ? profile.platforms.length : "—"}</strong>
          </div>
        </div>
        {createConversation.isError || follow.isError ? (
          <p className="profile-inline-error" role="alert">
            That action could not be completed. Please try again.
          </p>
        ) : null}
      </section>
      {own && completion.score < 100 && nextRequirement ? (
        <aside className="profile-completion" aria-label={`Profile ${completion.score}% complete`}>
          <span aria-hidden="true">{completion.score}%</span>
          <div>
            <strong>Your player identity is {completion.score}% complete</strong>
            <p>
              Still needed: {nextRequirement.label}
              {completion.missing.length > 1 ? ` and ${completion.missing.length - 1} more` : ""}.
            </p>
            <div
              role="progressbar"
            ...
            >
              <i style={{ width: `${completion.score}%` }} />
            </div>
          </div>
          <button type="button" onClick={() => navigate(nextRequirement.route)}>
            {nextRequirement.action}
          </button>
        </aside>
      ) : null}
      {network ? (
