  ...
  return <>
    <section className="profile-hero">
      ...
      <div className="profile-hero__identity">
        ...
        <div className="profile-hero__actions">{own ? <Button variant="quiet" onClick={() => navigate("/settings?section=profile")}><Settings size={16} />Edit profile</Button> : <><Button variant="quiet" disabled={createConversation.isPending} onClick={() => void message()}><MessageCircle size={16} />Message</Button><Button variant="secondary" onClick={() => navigate(`/challenges?compose=user&target=${encodeURIComponent(profile.id)}&targetName=${encodeURIComponent(profile.username)}`)}><Swords size={16} />Challenge</Button><Button disabled={follow.isPending} onClick={() => follow.mutate()}>{profile.isFollowedByCurrentUser ? <UserMinus size={16} /> : <UserPlus size={16} />}{profile.isFollowedByCurrentUser ? "Following" : "Follow"}</Button></>}</div>
      </div>
      <div className="profile-hero__metrics"><button type="button" aria-label={`${followers.toLocaleString()} followers. Open followers list`} aria-haspopup="dialog" aria-expanded={network === "followers"} onClick={() => setNetwork("followers")}><span>Followers</span><strong>{followers.toLocaleString()}</strong></button><button type="button" aria-label={`${following.toLocaleString()} following. Open following list`} aria-haspopup="dialog" aria-expanded={network === "following"} onClick={() => setNetwork("following")}><span>Following</span><strong>{following.toLocaleString()}</strong></button><div><span>Player level</span><strong>{profile.gameLevel || "Not set"}</strong></div><div><span>Platforms</span><strong>{profile.platforms?.length ? profile.platforms.length : "—"}</strong></div></div>
