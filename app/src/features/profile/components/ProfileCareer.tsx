...
const teamName = (team: ProfileTeam) =>
  team.name || team.team?.name || "Gamerie team";
        ...
        <div className="profile-career-teams">
          {teams.map((team, index) => {
            const membership = team.members?.find(
              (member) =>
                member.user?.id === profile.id || member.userId === profile.id,
            );
            return (
              <Link
                key={team.id || index}
                to={`/teams/${encodeURIComponent(teamSlug(team))}`}
              >
                <SafeImage
...
function TournamentsPanel({
  ...
  const sorted = useMemo(
    () =>
      tournaments
        ...
        .sort(
          (a, b) =>
            (tournamentDate(b.item)?.getTime() ?? 0) -
            (tournamentDate(a.item)?.getTime() ?? 0),
        ),
    [tournaments],
  );
  ...
  const begin = (value: number | "new") => {
    ...
    setDraft({
      ...
      game:
        tournamentGame(item) === "Game not shared" ? "" : tournamentGame(item),
      date: item.date?.slice(0, 10) || new Date().toISOString().slice(0, 10),
      status:
        item.status ||
        ((tournamentDate(item)?.getTime() ?? 0) > Date.now()
          ? "upcoming"
          : "completed"),
  ...
  const normalized = tournaments.map((item) => ({
    ...
    name: tournamentName(item),
    game:
      tournamentGame(item) === "Game not shared"
        ? "Other"
        : tournamentGame(item),
  ...
  const save = async () => {
    ...
    const next = {
      ...
      totalParticipants: draft.totalParticipants
        ? Number(draft.totalParticipants)
        : 1,
  ...
  return (
    <section className="profile-tournaments">
      <header>
        ...
        {own ? (
          <Button className="profile-career-add" size="small" variant="secondary" onClick={() => begin("new")}>
            <Plus size={13} />
            Add tournament
          </Button>
        ) : null}
            ...
            return (
              <article key={item.id || index}>
                ...
                <div className="profile-tournament-result">
                  ...
                  {item.prizePool != null ? (
                    <small>Pool {String(item.prizePool)}</small>
                  ) : null}
                </div>
                {item.url ? (
                  <a
      ...
      {update.isError ? (
        <p className="profile-inline-action-error" role="alert">
          {getApiErrorMessage(
            update.error,
            "Tournament history could not be updated.",
          )}
        </p>
      ) : null}
      {editing !== null ? (
          ...
          >
            ...
            {update.isError ? (
              <p className="profile-inline-action-error" role="alert">
                {getApiErrorMessage(
                  update.error,
                  "Tournament history could not be saved.",
                )}
              </p>
            ) : null}
            <footer>
...
export function ProfileCareer({
  ...
  return (
    <div className="profile-career-layout">
      ...
      {own ? <SalaryInsights profile={profile} /> : null}
    </div>
  );
}
