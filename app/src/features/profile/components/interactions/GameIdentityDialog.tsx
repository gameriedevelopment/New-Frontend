...
export function GameIdentityDialog({
  game,
  onClose,
  profile,
}: {
  ...
  const [form, setForm] = useState({
    gameId: game?.gameId || game?.game?.id || "",
    gameName: game?.name || game?.game?.name || "",
    gameUsername: game?.gameUsername || "",
    platform: game?.platform || game?.platforms?.[0] || "",
    skillLevel: game?.skillLevel || "",
    rank: game?.rankData?.rank || game?.rank || "",
  ...
  const options =
    catalogue.data?.pages
      ...
      .filter(
        (game) =>
          !(profile.gamesPlayed ?? profile.games ?? []).some(
            (saved) => String(saved.gameId || saved.game?.id) === String(game.id),
          ),
      ) ?? [];
  const submit = async () => {
    const existing = (
      profile.gamesPlayed?.length ? profile.gamesPlayed : (profile.games ?? [])
    ).map(existingGamePayload);
    const next = {
      id: game?.id,
      gameId: form.gameId,
      name: form.gameName,
    ...
    const isEditedGame = (item: ReturnType<typeof existingGamePayload>) =>
      item.id && game?.id ? item.id === game.id : String(item.gameId) === String(form.gameId);
    await update.mutateAsync({
      gamesPlayed: game
        ? existing.map((item) => (isEditedGame(item) ? next : item))
        : [...existing, next],
    });
    onClose();
  };
  return (
    <ProfileDialog
      title={game ? "Edit game identity" : "Add a game"}
    ...
    >
      ...
      >
        {game ? (
          <div className="profile-game-selection">
            <span>Game</span>
            <strong>{form.gameName || "Connected game"}</strong>
          </div>
        ) : (
          <SearchSelect
            label="Game"
            value={form.gameId}
            onChange={(gameId) => {
              const selected = options.find((item) => item.id === gameId);
              setForm((current) => ({ ...current, gameId, gameName: selected?.name || "" }));
            }}
            onSearch={onSearch}
            loading={catalogue.isLoading}
            loadingMore={catalogue.isFetchingNextPage}
        ...
        <label>
          <span>In-game username or player tag</span>
          <input
            value={form.gameUsername}
            maxLength={80}
            autoComplete="off"
            onChange={(event) =>
              setForm((current) => ({ ...current, gameUsername: event.target.value }))
            }
            placeholder="Your identity in this game"
          />
        </label>
        ...
        <label>
          <span>
            Current rank <small>Optional</small>
          </span>
          <input
            value={form.rank}
            maxLength={80}
        ...
        {update.isError ? (
          <p className="profile-inline-action-error" role="alert">
            {getApiErrorMessage(update.error, "This game could not be added to your profile.")}
          </p>
        ) : null}
        <footer>
          ...
          <Button
            ...
            disabled={
              update.isPending ||
              !form.gameId ||
              !form.gameUsername.trim() ||
              !form.platform ||
              !form.skillLevel
            }
          >
            {update.isPending ? "Saving…" : game ? "Save game" : "Add game"}
          </Button>
        </footer>
      </form>
    </ProfileDialog>
