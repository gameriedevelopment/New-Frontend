...
export function MilestonesPanel({ own, profile }: { own: boolean; profile: PlayerProfile }) {
  ...
  const source =
    editing === "new" || editing === null ? empty : (profile.milestones?.[editing] ?? empty);
  const [draft, setDraft] = useState<Milestone>(empty);
  const normalized = (items: NonNullable<PlayerProfile["milestones"]>) =>
    items.map((item) => ({
      title: item.title || "Milestone",
      description: item.description || "",
      date: String(item.date || item.createdAt || new Date().toISOString()).slice(0, 10),
      type: types.includes(item.type as (typeof types)[number]) ? item.type : "other",
      ...(item.icon ? { icon: item.icon } : {}),
    }));
  const begin = (index: number | "new") => {
    const current = index === "new" ? empty : (profile.milestones?.[index] ?? empty);
    setDraft({
      ...current,
      date: String(current.date || current.createdAt || new Date().toISOString()).slice(0, 10),
      type: types.includes(current.type as (typeof types)[number]) ? current.type : "other",
    });
    setEditing(index);
  };
  const save = async () => {
    const items = normalized(profile.milestones ?? []);
    const value = {
    ...
    if (editing === "new") items.push(value);
    else if (typeof editing === "number") items[editing] = value;
    await update.mutateAsync({ milestones: items });
    setEditing(null);
  };
  const remove = async (index: number) => {
    await update.mutateAsync({
      milestones: normalized(profile.milestones ?? []).filter(
  ...
  const visible = expanded ? milestones : milestones.slice(0, 4);
  return (
      ...
      {milestones.length ? (
        <>
          <div className="profile-milestones__timeline">
            {visible.map((item, index) => (
              <article key={item.id || index} data-type={item.type || "career"}>
                ...
                <div>
                  <span>{item.type || "Career"}</span>
                  <h3>{item.title || "Milestone"}</h3>
          ...
          {milestones.length > 4 ? (
            ...
            >
              {expanded ? "Show fewer milestones" : `View all ${milestones.length} milestones`}
            </button>
          ) : null}
        </>
      ) : (
        <p className="profile-panel-empty">
          {own
            ? "Build a credible career record with meaningful team, competition, and personal milestones."
            : "This player has not published career milestones yet."}
        </p>
      )}
      {update.isError && editing === null ? (
        <p className="profile-inline-action-error" role="alert">
          {getApiErrorMessage(update.error, "The milestone could not be updated.")}
        </p>
      ) : null}
      {editing !== null ? (
        <ProfileDialog
          title={editing === "new" ? "Add career milestone" : "Edit career milestone"}
          ...
          >
            ...
            <label>
              ...
              <input
                ...
                placeholder="Joined a competitive roster"
              />
            </label>
            <label>
              <span>Date</span>
            ...
            {update.isError ? (
              <p className="profile-inline-action-error" role="alert">
                {getApiErrorMessage(update.error, "The milestone could not be saved.")}
              </p>
            ) : null}
            <footer>
