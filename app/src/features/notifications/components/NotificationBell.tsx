...
export function NotificationBell() {
  ...
  useEffect(() => {
    ...
    if (keyboardOpenRef.current)
      requestAnimationFrame(() =>
        panelRef.current?.querySelector<HTMLButtonElement>(".notification-item")?.focus(),
      );
    return () => {
  ...
  return (
    <div className="notification-bell" ref={rootRef}>
      ...
      >
        <Bell size={18} />
        {unread ? <span className="app-header__badge">{badge}</span> : null}
      </button>
      {open ? (
        <div
        ...
        >
          ...
          {markAllRead.isError ? (
            <p className="notification-menu__error">
          ...
          <div className="notification-menu__list">
            {menuQuery.isLoading
              ? Array.from({ length: 4 }, (_, index) => (
                  <div className="notification-item-skeleton" key={index}>
            ...
            {menuQuery.isError ? (
              <div className="notification-menu__state">
                ...
                <button type="button" onClick={() => menuQuery.refetch()}>
                  Try again
                </button>
              </div>
            ) : null}
            {!menuQuery.isLoading && !menuQuery.isError && !menu.length ? (
              <div className="notification-menu__state">
                <Bell size={18} />
