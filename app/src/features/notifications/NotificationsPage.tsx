...
const filters: Array<{ value: NotificationFilter; label: string }> = [
  ...
  { value: "teams", label: "Teams & hubs" },
  { value: "competitive", label: "Matches & challenges" },
  { value: "wallet", label: "Wallet" },
  { value: "updates", label: "Gamerie updates" },
];
...
function NotificationPageSkeleton() {
  return (
    <div className="notifications-skeleton" aria-label="Loading notifications" role="status">
      {Array.from({ length: 6 }, (_, index) => (
        <div key={index}>
          <SkeletonAvatar size={40} />
          <span>
            <Skeleton width={index % 2 ? "38%" : "28%"} height={9} />
...
export function NotificationsPage() {
  ...
  const notifications = flattenNotifications(query.data).filter(
    (notification) => !unreadOnly || !notification.isRead,
  );
  const unread = unreadQuery.data ?? 0;
  const groups = useMemo(
    () =>
      notifications.reduce<Record<string, GamerieNotification[]>>((result, notification) => {
  ...
  const setStatus = (status: "all" | "unread") => {
    const next = new URLSearchParams(params);
    if (status === "unread") next.set("status", "unread");
  ...
  const open = (notification: GamerieNotification) => {
    ...
    <section className="notifications-page">
      <header>
        <div>
          <p>Activity</p>
          <h1>Notifications</h1>
          <span>Follow the moments that need your attention without losing your place.</span>
        </div>
        {unread > 0 ? (
          <div className="notifications-page__actions">
            <strong>{unread > 99 ? "99+" : unread} unread</strong>
            <button
              type="button"
      ...
      {markAllRead.isError ? (
        <p className="notifications-page__error" role="alert">
          Your notifications could not all be marked as read. Please try again.
        </p>
      ) : null}
      <div className="notifications-controls">
        <div role="tablist" aria-label="Read status">
          <button
            type="button"
            role="tab"
      ...
      {query.isError ? (
        <StatePanel
          ...
          description="Gamerie could not reach your activity right now."
          action={
            <Button size="small" variant="quiet" onClick={() => query.refetch()}>
              <RefreshCw size={14} />
              Try again
            </Button>
          }
      ...
      {!query.isLoading && !query.isError && !notifications.length ? (
        <div className="notifications-empty">
          ...
          <p>
            {unreadOnly
              ? "You have reviewed every notification currently waiting for you."
              : "New activity will appear here as you connect, play, and build your place on Gamerie."}
          </p>
        </div>
      ) : null}
      {!query.isLoading && !query.isError ? (
        <div className="notifications-groups">
          {["Today", "Yesterday", "Earlier"].map((name) =>
            groups[name]?.length ? (
              <section key={name}>
                <h2>{name}</h2>
                <div>
                  {groups[name].map((notification) => (
                    <NotificationItem
                      key={notification.id}
      ...
      <div className="notifications-load" ref={loadRef}>
