...
export function readCookieConsent(): CookieConsentChoice | null {
  ...
  try {
    const parsed = JSON.parse(decodeURIComponent(value)) as Partial<CookieConsentChoice>;
    if (
      parsed.version !== 1 ||
...
export function persistCookieConsent(choice: Pick<CookieConsentChoice, "analytics" | "marketing">) {
