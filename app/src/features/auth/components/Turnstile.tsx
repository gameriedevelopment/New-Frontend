...
interface TurnstileApi {
  render: (element: HTMLElement, options: { sitekey: string; callback: (token: string) => void; "expired-callback": () => void; "error-callback": () => void; theme: "dark"; appearance: "interaction-only" }) => string;
...
export function Turnstile({ onVerify, onExpire }: { onVerify: (token: string) => void; onExpire: () => void }) {
  ...
  useEffect(() => {
    ...
    loadTurnstile().then(() => {
      ...
      widgetRef.current = window.turnstile.render(element, { sitekey: siteKey, theme: "dark", appearance: "interaction-only", callback: onVerify, "expired-callback": onExpire, "error-callback": onExpire });
    }).catch(onExpire);
    return () => {
      cancelled = true;
