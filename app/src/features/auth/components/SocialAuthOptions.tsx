...
export function SocialAuthOptions({ mode }: { mode: "signin" | "signup" }) {
  ...
  return (
    <section className="auth-social" aria-label={`${mode === "signin" ? "Sign in" : "Sign up"} with a connected account`}>
      <div className="auth-social__divider"><span>or continue with</span></div>
      <div className="auth-social__options">
        {providers.map((provider) => (
          <button key={provider.id} type="button" onClick={() => continueWith(provider.id)} disabled={!provider.available || activeProvider !== null} aria-label={`${mode === "signin" ? "Sign in" : "Sign up"} with ${provider.label}${provider.available ? "" : ", unavailable"}`} title={provider.available ? undefined : "Apple sign in is not available yet"}>
            <img src={provider.iconSrc} alt="" />
            <span>{provider.label}</span>
            {activeProvider === provider.id && <i className="auth-social__loader" aria-hidden="true" />}
          </button>
        ))}
      </div>
    </section>
