...
export function AuthCallbackPage() {
  ...
  const [callbackError, setCallbackError] = useState(
    "We couldn't complete this sign in. Please try again.",
  );
  ...
  useEffect(() => {
    ...
    const completeCallback = async () => {
      try {
        ...
        setCallbackError(
          getApiErrorMessage(error, "We couldn't complete this sign in. Please try again."),
        );
        setState("failed");
      }
    };
  ...
  const submitProfile = handleSubmit(async (values) => {
    try {
      ...
      setCallbackError(
        getApiErrorMessage(error, "We couldn't save your gaming profile. Try again."),
      );
    }
  });
  ...
  return (
    <main className="auth-entry">
      ...
      <section
        className={`auth-stage${state === "profile" ? " auth-stage--register" : ""}`}
      ...
      >
        ...
        <section
          className={`auth-panel${state === "profile" ? " auth-panel--register" : " auth-process"}`}
        >
          {state === "working" && (
            <>
              <span className="auth-process__loader" aria-hidden="true" />
              <h2 id="callback-title">Completing sign in.</h2>
              <p>Connecting your account securely.</p>
            </>
          )}
          {state === "failed" && (
            <>
              <AlertCircle className="auth-process__error-mark" size={20} />
              <h2 id="callback-title">Sign in not completed.</h2>
              <p>{callbackError}</p>
          ...
          {state === "profile" && (
            <>
              <div className="auth-panel__header">
                <h2 id="callback-title">Complete your profile.</h2>
                <p>Choose how you want to show up across Gamerie.</p>
              </div>
              <form className="auth-form" onSubmit={submitProfile} noValidate>
                ...
                <div className="auth-field" data-invalid={Boolean(errors.gamerTitle)}>
                  <label htmlFor="social-gamer-title">What best describes you?</label>
                  <select id="social-gamer-title" defaultValue="" {...register("gamerTitle")}>
                ...
                <div className="auth-field" data-invalid={Boolean(errors.gameLevel)}>
                  <label htmlFor="social-game-level">Gaming level</label>
                  <select id="social-game-level" defaultValue="" {...register("gameLevel")}>
                    <option value="" disabled>
                      Select one
                    </option>
                    {levels.map((level) => (
                ...
                <button className="auth-submit" type="submit" disabled={isSubmitting}>
                  {isSubmitting ? (
                    ...
                    <>
                      Continue to Gamerie <ArrowRight size={17} />
                    </>
                  )}
                </button>
              </form>
