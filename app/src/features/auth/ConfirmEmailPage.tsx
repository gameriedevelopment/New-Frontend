    ...
    <main className="auth-entry">
      ...
      <section className="auth-stage" aria-labelledby="confirm-email-title">
        ...
        <section className="auth-panel auth-process">
          {state.status === "working" ? <><span className="auth-process__loader" aria-hidden="true" /><h2 id="confirm-email-title">Confirming your email.</h2><p>Please keep this page open for a moment.</p></> : state.status === "done" ? <><h2 id="confirm-email-title">Email confirmed.</h2><p>Your email address is verified and your account is ready for the next step.</p><Link className="auth-submit auth-process__action" to={state.next}>{state.next === "/verify-phone" ? "Verify phone number" : "Continue to Gamerie"}<ArrowRight size={17} /></Link></> : <><AlertCircle className="auth-process__error-mark" size={20} /><h2 id="confirm-email-title">Link not confirmed.</h2><p>{state.message}</p><Link className="auth-submit auth-process__action" to="/login">Back to sign in <ArrowRight size={17} /></Link></>}
        </section>
      </section>
    </main>
  );
