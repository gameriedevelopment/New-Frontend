...
export function PasswordRecoveryPage() {
  ...
  const confirmForm = useForm<ConfirmFields>({
  ...
  const submitRequest = requestForm.handleSubmit(async ({ email }) => {
    ...
    try {
      ...
      setSubmitError(
        getApiErrorMessage(error, "We couldn't send the reset link. Please try again."),
      );
    }
  });
        ...
        <section className="auth-panel auth-recovery">
          {result === "request-sent" ? (
            <>
              ...
              <p>
                If an account exists for <strong>{requestEmail}</strong>, a password-reset link is
                on its way.
              </p>
              <p className="auth-recovery__note">
                The link may take a few minutes to arrive. Check your spam folder if you do not see
                it.
              </p>
            ...
            <>
              ...
              <Link className="auth-submit auth-recovery__primary" to="/login">
            ...
            <>
              <div className="auth-panel__header">
                <h2 id="recovery-title">
                  {isConfirmMode ? "Create a new password." : "Reset your password."}
                </h2>
                <p>
                  {isConfirmMode
              ...
              <form
                className="auth-form"
                onSubmit={isConfirmMode ? submitReset : submitRequest}
                noValidate
              >
                {submitError && (
                  <div className="auth-error" role="alert">
                    <AlertCircle size={17} />
                    <p>
                      <strong>{isConfirmMode ? "Password not updated" : "Link not sent"}</strong>
                      {submitError}
                    </p>
                  </div>
                )}
                ...
                {isConfirmMode ? (
                  <>
                    <div
                      ...
                      data-invalid={Boolean(confirmForm.formState.errors.password)}
                    >
                      ...
                      {confirmForm.formState.errors.password ? (
                        <small role="alert">{confirmForm.formState.errors.password.message}</small>
                      ) : (
                    ...
                    <div
                      className="auth-field"
                      data-invalid={Boolean(confirmForm.formState.errors.confirmPassword)}
                    >
                  ...
                  <>
                    <div
                      className="auth-field"
                      data-invalid={Boolean(requestForm.formState.errors.email)}
                    >
                ...
                >
                  {isSubmitting ? (
                    <>
                      ...
                      {isConfirmMode ? "Updating password…" : "Sending link…"}
                    </>
                  ) : (
                    <>
                ...
                <Link className="auth-back-link auth-back-link--form" to="/login">
                  <ArrowLeft size={15} />
                  Back to sign in
                </Link>
              </form>
