...
export function PlayerCard({ player }: { player: PlayerProfile }) {
  ...
  const follow = useTogglePlayerFollow(
    viewer?.id,
    player.id,
    Boolean(player.isFollowedByCurrentUser),
  );
  ...
  return (
    <article className="player-card">
      <header>
        <Link className="player-card__identity" to={`/profile/${player.username}`}>
          <span>
            <SafeImage src={player.profileImage} alt="" />
            {player.isOnline ? <i aria-label="Online now" /> : null}
          </span>
          <div>
            <h2>{player.username}</h2>
            <p>{player.gamerTitle || "Gamerie player"}</p>
          </div>
        </Link>
        {viewer?.id !== player.id ? (
          <Button
            size="small"
            variant={player.isFollowedByCurrentUser ? "quiet" : "secondary"}
            disabled={follow.isPending}
            aria-label={`${player.isFollowedByCurrentUser ? "Unfollow" : "Follow"} ${player.username}`}
            onClick={() => follow.mutate()}
          >
            {follow.isPending
              ? "Updating…"
              : player.isFollowedByCurrentUser
                ? "Following"
                : "Follow"}
          </Button>
        ) : null}
      ...
      <div className="player-card__context">
        <div>
          <Gamepad2 size={14} />
          <span>
            <small>Primary game</small>
            <strong>
              {primaryGame || (games.length ? `${games.length} connected games` : "Not shared")}
            </strong>
          </span>
        </div>
        <div>
          ...
          <span>
            ...
            <strong>{player.gameLevel || "Open profile"}</strong>
          </span>
        </div>
        {platform ? (
          <div>
      ...
      <footer>
        ...
        <Link to={`/profile/${player.username}`} aria-label={`Open ${player.username}'s profile`}>
