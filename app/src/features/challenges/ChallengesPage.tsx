...
const isOwnerTeam = (team: ProfileTeam, userId: string) =>
  team.role === "owner" ||
  team.title === "owner" ||
  Boolean(
    team.members?.some(
      (member) =>
        (member.userId === userId || member.user?.id === userId) && member.role === "owner",
    ),
  );
...
function ChallengeListSkeleton() {
  return (
    <div className="challenge-list" aria-label="Loading challenges">
...
export function ChallengesPage() {
  ...
  const scope = (
    views.some((view) => view.value === params.get("view")) ? params.get("view") : "for-you"
  ) as ChallengeScope;
  const direction = (
    ["all", "received", "sent"].includes(params.get("direction") || "")
      ? params.get("direction")
      : "all"
  ) as ChallengeDirection;
  const status = (params.get("status") || "all") as ChallengeStatus | "all";
  const [search, setSearch] = useState(params.get("q") || "");
  const debounced = useDebouncedValue(search, 280);
  const [filtersOpen, setFiltersOpen] = useState(
    () => params.has("direction") || params.has("status"),
  );
  ...
  const ownedTeams = useMemo(
    () => (profiles.data || []).filter((team) => isOwnerTeam(team, user?.id || "")),
    [profiles.data, user?.id],
  );
  const ownedTeamIds = useMemo(() => ownedTeams.map(teamId).filter(Boolean), [ownedTeams]);
  const filters = useMemo<ChallengeFilters>(
    () => ({ scope, direction, status, searchTerm: debounced || undefined }),
    [debounced, direction, scope, status],
  );
  ...
  const clear = () => {
    ...
    const next = new URLSearchParams(params);
    ["q", "status", "direction"].forEach((key) => next.delete(key));
    setParams(next, { replace: true });
  };
  ...
  const closeOverlay = useCallback(() => {
    if (route.challengeId) navigate("/challenges");
    else {
      const next = new URLSearchParams(params);
      ["challenge", "compose", "target", "targetName"].forEach((key) => next.delete(key));
      setParams(next, { replace: true });
    }
  }, [navigate, params, route.challengeId, setParams]);
  const openComposer = () => {
    const next = new URLSearchParams(params);
    next.set("compose", "user");
    next.delete("challenge");
    setParams(next, { replace: true });
  };
  ...
  return (
    <main className="challenges-page">
      ...
      <nav className="challenge-views" aria-label="Challenge views">
      ...
      <section className="challenge-toolbar" aria-label="Challenge filters">
        <div>
          <Search size={16} />
          <label>
          ...
          <button
            ...
            aria-controls="challenge-filters"
        ...
        <div id="challenge-filters" hidden={!filtersOpen}>
          <label>
            <span>Direction</span>
            <select
              value={direction}
              onChange={(event) =>
                change("direction", event.target.value === "all" ? undefined : event.target.value)
              }
            >
          ...
          {scope !== "history" ? (
            <label>
              ...
              <select
                value={status}
                onChange={(event) =>
                  change("status", event.target.value === "all" ? undefined : event.target.value)
                }
              >
                <option value="all">Pending and accepted</option>
      ...
      {query.isLoading ? (
        ...
        <StatePanel
          ...
          description={getApiErrorMessage(
            query.error,
            "Gamerie could not retrieve your competition inbox.",
          )}
          action={
        ...
        <>
          <div className="challenge-list">
            {challenges.map((challenge) => (
              <ChallengeCard
                ...
                pending={update.isPending && update.variables?.id === challenge.id}
                onOpen={() => openChallenge(challenge.id)}
                onRespond={(nextStatus) =>
                  update.mutate({
                    id: challenge.id,
                    type: challenge.type,
          ...
          {update.isError ? (
            <p className="challenge-error" role="alert">
              {getApiErrorMessage(update.error, "The challenge could not be updated.")}
            </p>
          ) : null}
          <InfiniteLoadTrigger
        ...
        <StatePanel
          icon={<Swords size={20} />}
          title={scope === "history" ? "No challenge history yet" : "This challenge view is clear"}
          description={
            scope === "team" && !ownedTeams.length
              ? "Team challenges appear here when you own a team. Create or take ownership of a team to compete as a roster."
              : "New invitations and scheduled challenges will appear here as your competitive activity grows."
          }
          action={
            <Button onClick={openComposer}>
              <Plus size={14} />
      ...
      {composing && user?.id ? (
        <ChallengeComposer
          ...
          initialTargetId={params.get("target") || ""}
          initialTargetName={params.get("targetName") || ""}
          onClose={closeOverlay}
          onCreated={(id) => {
            const next = new URLSearchParams(params);
            ["compose", "target", "targetName"].forEach((key) => next.delete(key));
            next.set("challenge", id);
            setParams(next, { replace: true });
          }}
        />
