...
const memberId = (member: CommunityMember) =>
...
export function SensitiveCommunityOperations({
    ...
    () =>
      (item.members ?? []).filter(
        (member) =>
          ...
          memberId(member) &&
          memberId(member) !== item.ownerId,
      ),
    [item.members, item.ownerId],
  );
  const selectedMember = eligibleMembers.find(
    (member) => memberId(member) === newOwnerId,
  );
  const glk = wallet.data?.balances?.find(
    (balance) =>
      balance.tokenId === "glk-token" ||
      balance.token?.symbol?.toLowerCase() === "glk",
  );
  ...
  return (
    <>
      {kind === "team" ? (
        <section
          className="community-section community-sensitive__wallet"
          id="team-wallet"
        >
          <header>
            <div>
              <h2>Team wallet</h2>
              <p>Owner-only balance and challenge reservations.</p>
            </div>
            <WalletCards size={17} />
          </header>
          {wallet.isLoading ? (
            ...
            <>
              <div className="community-wallet-summary">
                <div>
                  ...
                  <strong>{Number(glk?.amount || 0).toLocaleString()}</strong>
                </div>
                <button
                  type="button"
                  onClick={copyAddress}
                ...
                >
                  ...
                  <strong>
                    {copied ? "Copied" : compactAddress(wallet.data.address)}
                  </strong>
                ...
                {reservations.length ? (
                  reservations.map(([challengeId, reservation]) => (
                    <article key={challengeId}>
                      ...
                      <div>
                        <strong>
                          {Number(reservation.amount || 0).toLocaleString()} GLK
                        </strong>
              ...
              <p className="community-sensitive__note">
                Reservations are created and released by the verified challenge
                lifecycle. Manual release is intentionally unavailable here.
              </p>
            </>
          ) : null}
        </section>
      ...
      <section
        className="community-section community-sensitive"
        id={`${kind}-sensitive`}
      >
        <header>
          <div>
            ...
            <p>
              These changes affect every member and cannot be silently reversed.
            </p>
          </div>
        </header>
        <div className="community-sensitive__actions">
          <div>
            ...
            <p>
              Choose an active member to become the new owner. You will remain
              as a regular {kind === "team" ? "member" : "administrator"}.
            </p>
            <Button
              variant="secondary"
            ...
            {!eligibleMembers.length ? (
              <small>
                Add another active member before ownership can be transferred.
              </small>
            ) : null}
          </div>
          <div className="community-sensitive__danger">
            ...
            <p>
              Permanently remove this {kind}, its membership, content, and
              connected records.
            </p>
      ...
      {dialog === "transfer" ? (
        ...
        >
          <div className="community-dialog__body">
            <p>
              Select the new owner, then type <strong>{item.name}</strong> to
              confirm. Ownership permissions change immediately.
            </p>
            <label>
              <span>New owner</span>
              <select
          ...
          <footer>
            ...
            <Button
              disabled={
                !newOwnerId || confirmation !== item.name || transfer.isPending
              }
              onClick={transferNow}
            >
      ...
      {dialog === "delete" ? (
        ...
        >
          <div className="community-dialog__body">
            <p>
              This permanently deletes the {kind}, its posts, membership,
              requests, and connected records. Type <strong>{item.name}</strong>{" "}
              to continue.
            </p>
            <label>
            ...
            {remove.isError ? (
              <p className="community-action-error" role="alert">
                {getApiErrorMessage(
                  remove.error,
                  `This ${kind} could not be deleted.`,
                )}
              </p>
            ) : null}
          </div>
