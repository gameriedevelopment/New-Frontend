  ...
  return (
    <div className="team-operation-person">
      ...
      <div>
        ...
        <span>{user?.username ? `@${user.username}` : request.message || "Player"}</span>
      </div>
    </div>
  );
}

export function TeamOperations({ team, slug }: { team: TeamSummary; slug: string }) {
  ...
  const moveView = (event: KeyboardEvent<HTMLButtonElement>, index: number) => {
    ...
    changeView(nextView);
    requestAnimationFrame(() => document.getElementById(`team-manage-tab-${nextView}`)?.focus());
  };
  ...
  return (
    <div className="team-operations">
      <div className="community-manage-tabs" role="tablist" aria-label="Team management sections">
        ...
        <section className="community-section team-operations__invite">
          <header>
            <div>
              ...
              <p>Search by username, confirm the player, then send an invitation.</p>
            </div>
            <MailPlus size={17} />
          </header>
          <form onSubmit={submitInvite}>
            <label>
              ...
              <input
                ...
                autoComplete="off"
              />
              <Button size="small" disabled={!inviteName.trim() || invite.isPending}>
          ...
          {debouncedName ? (
            <div className="team-invite-results" aria-live="polite">
              {search.isLoading ? (
                ...
                <p>No matching players found. You can still send an exact username.</p>
              )}
            </div>
          ) : null}
          {invite.isError ? (
            <p className="community-action-error" role="alert">
              {getApiErrorMessage(invite.error, "The invitation could not be sent.")}
            </p>
          ) : invite.isSuccess ? (
        ...
        <div className="team-operations__queues">
          <section className="community-section">
            ...
            {requests.isLoading ? (
              ...
              <p className="community-section__empty">No join requests need attention.</p>
            )}
            {respond.isError ? (
              <p className="community-action-error" role="alert">
                {getApiErrorMessage(respond.error, "This request could not be updated.")}
              </p>
            ) : null}
          </section>
          <section className="community-section">
            ...
            {invites.isLoading ? (
                ...
                {invites.data.map((request) => (
                  <article key={request.id}>
                    ...
                    <Button
                      ...
                      onClick={() => request.user?.id && cancel.mutate(request.user.id)}
                    >
                      Cancel invite
                    </Button>
                  </article>
              ...
              <p className="community-section__empty">No invitations are waiting.</p>
            )}
            {cancel.isError ? (
              <p className="community-action-error" role="alert">
                {getApiErrorMessage(cancel.error, "This invitation could not be cancelled.")}
              </p>
            ) : null}
          </section>
        </div>
        ...
        <section className="community-section">
          <header>
            <div>
              ...
              <p>Owners set titles; owners and managers can remove roster members.</p>
            </div>
          ...
          {roster.length ? (
            <div className="team-roster-controls">
              {roster.map((member, index) => {
                ...
                const isCreator = member.role === "owner" || userId === team.ownerId;
                return (
                    ...
                    {!isCreator ? (
                      <div>
                        {relationship.isOwner ? (
                          <Button size="small" variant="quiet" onClick={() => openEdit(member)}>
                            Edit title
                          </Button>
                        ) : null}
                        <Button size="small" variant="quiet" onClick={() => setRemoving(member)}>
                          <UserMinus size={13} />
                          Remove
                        </Button>
                      </div>
        ...
        <div className="team-operations__queues">
          <section className="community-section">
            ...
            {ranks.isLoading ? (
              ...
              <p className="community-section__empty">Rankings are temporarily unavailable.</p>
            ) : ranks.data?.length ? (
              <div className="team-rank-list">
                {ranks.data.map((entry, index) => (
                  <div key={entry.id || index}>
                    <span>{typeof entry.game === "string" ? entry.game : entry.game.name}</span>
                    <strong>
                      {entry.rankData?.tier
            ...
            <Link className="team-operation-link" to={`/teams/${encodeURIComponent(slug)}/edit`}>
              <Gamepad2 size={14} />
              Manage team games
              <ChevronRight size={14} />
            </Link>
        ...
        >
          <div className="community-dialog__body">
            <p>
              ...
              {editing.user?.displayName || editing.user?.username || "this member"}.
            </p>
            <label>
            ...
            {changeMember.isError ? (
              <p className="community-action-error" role="alert">
                {getApiErrorMessage(changeMember.error, "The member title could not be updated.")}
              </p>
            ) : null}
          </div>
          <footer>
      ...
      {removing ? (
        ...
        >
          <div className="community-dialog__body">
            <p>
              {removing.user?.displayName || removing.user?.username || "This player"} will lose
              access to the team roster and its member-only activity.
            </p>
