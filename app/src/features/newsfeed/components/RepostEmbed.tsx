...
export function RepostEmbed({ post }: { post: FeedPost }) {
  ...
  return <Link className="repost-embed" to={href}>
    <header><SafeImage src={author.image} alt="" /><strong>{author.name}</strong><span>Original post</span></header>
    {post.content ? <RichText className="repost-embed__content" content={post.content} /> : null}
    {media ? <SafeImage className="repost-embed__media" src={media} fallback="/media-fallback.svg" alt="Original post media" /> : null}
  </Link>;
}
