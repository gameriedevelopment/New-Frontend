...
function totalLikes(post: FeedPost) {
  ...
  return Object.values(post.reactionCounts ?? {}).reduce(
...
export function FeedPostCard({
  ...
  const toggleLike = async () => {
    ...
    try {
      ...
      if (!wasLiked && user?.id && targetId && targetId !== user.id)
        notifyPostLike({
          userId: user.id,
          targetId,
          contentType: "post",
      ...
      /* Optimistic state is rolled back by the mutation. */
    }
  };
  ...
  const openPost = (target: EventTarget | null) => {
    if (
      single ||
      (target instanceof Element && target.closest("a, button, input, textarea, select, video"))
    )
      return;
    navigate(postHref);
  };
  ...
  return (
    <article
      className="feed-post"
      ref={postRef}
      data-clickable={!single || undefined}
      onClick={(event) => openPost(event.target)}
    >
      <header className="feed-post__header">
        <Link
          className="feed-post__identity"
          to={author.username ? `/profile/${author.username}` : `/users/${author.id}`}
        >
          {author.image ? (
            <SafeImage src={author.image} alt="" />
          ) : (
            <span className="feed-avatar">{author.name.slice(0, 2).toUpperCase()}</span>
          )}
          <span>
        ...
        {single ? (
          ...
          <Link className="feed-post__time" to={postHref} aria-label="Open post conversation">
      ...
      {post.isAnnouncement ? (
        <div className="feed-post__announcement">
          <Megaphone size={14} />
          Announcement
        </div>
      ) : null}
      {post.repostOf ? (
        <div className="feed-post__repost">
          <span>
            <Repeat2 size={14} />
            Reposted
          </span>
          {post.content ? <PostContent content={post.content} single={single} /> : null}
          <RepostEmbed post={post.repostOf} />
        </div>
      ) : post.content ? (
        <>
          <PostContent content={post.content} single={single} />
          <LinkPreviewCard content={post.content} />
        </>
      ) : null}
      {!post.repostOf && media.length ? (
        <PostMediaGallery media={media} mediaType={post.mediaType} />
      ) : null}
      <footer className="feed-post__actions" data-count={canRepost ? 4 : 3}>
        ...
        {canRepost ? (
          <button
            type="button"
            className={reposted ? "is-active" : undefined}
        ...
        <button type="button" onClick={sharePost}>
          <Share2 size={18} />
          <span>
            {shareStatus === "copied" ? "Copied" : shareStatus === "error" ? "Try again" : "Share"}
          </span>
        </button>
      </footer>
