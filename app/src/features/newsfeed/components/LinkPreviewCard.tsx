...
export function LinkPreviewCard({
  ...
  if (!url) return null;
  if (query.isLoading && onDismiss)
    return (
      <div className="link-preview-loading" aria-label="Loading link preview">
        <div>
  ...
  if (query.isLoading || query.isError || !query.data || (!query.data.title && !query.data.image))
    return null;
  ...
  try {
    ...
    /* Invalid URLs render as external. */
  }
  const body = (
    <>
      <div className="link-preview__body">
        {preview.siteName ? <span>{preview.siteName}</span> : null}
      ...
      {preview.image ? (
        <SafeImage src={preview.image} fallback="/media-fallback.svg" alt="" />
      ) : null}
    </>
  );
  const card = internalPath ? (
    <Link className="link-preview" to={internalPath}>
      {body}
    </Link>
  ) : (
  ...
  return onDismiss ? (
    <div className="link-preview-wrap">
      {card}
      <button type="button" onClick={onDismiss} aria-label="Remove link preview">
        ×
      </button>
    </div>
  ) : (
