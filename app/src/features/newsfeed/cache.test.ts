...
describe("newsfeed post cache interactions", () => {
  it("patches feed pages, single posts, and embedded reposts consistently", () => {
    ...
    const feed = {
      pages: [{ posts: [post()], nextCursor: null } satisfies FeedPage],
      pageParams: [null],
    };
    const embedded = post({ id: "repost-1", repostOf: post() });

    expect(patchPostCacheValue(feed, "post-1", update)).toMatchObject({
      pages: [{ posts: [{ content: "Updated" }] }],
    });
    expect(patchPostCacheValue(post(), "post-1", update)).toMatchObject({ content: "Updated" });
    expect(patchPostCacheValue(embedded, "post-1", update)).toMatchObject({
      repostOf: { content: "Updated" },
    });
  });
