...
export function SinglePostPage() {
  ...
  const resolvedType: PostOwnerType =
    hubId || query.data?.hubId ? "hub" : teamId || query.data?.teamId ? "team" : "user";
  ...
  const canDeleteAsCommunityOwner = Boolean(
    user?.id && (hubContext.data?.ownerId === user.id || teamContext.data?.ownerId === user.id),
  );
  ...
  return (
    <div className="single-post-page">
      <header className="single-post-page__header">
        <Link to="/feed">
          <ArrowLeft size={16} />
          Back to feed
        </Link>
      ...
      {query.isError || (!query.isLoading && !query.data) ? (
        <StatePanel
          ...
          description="It may have been removed, or Gamerie could not load it right now."
          action={
      ...
      {query.data ? (
        <>
          <FeedPostCard post={query.data} single canDeleteOverride={canDeleteAsCommunityOwner} />
          {isPostAuthor ? <PostOwnerInsights post={query.data} /> : null}
          <CommentSection post={query.data} type={resolvedType} />
        </>
      ) : null}
    </div>
  );
