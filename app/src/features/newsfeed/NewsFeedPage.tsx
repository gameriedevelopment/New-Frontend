        ...
        <section className="feed-stream" aria-label="Posts">
          ...
          {query.isError ? (
            <StatePanel
              ...
              description="We could not reach Gamerie right now. Your account and content are safe."
              action={
            ...
            <StatePanel
              title={
                filter === "all"
                  ...
                  : `Nothing in ${filters.find((item) => item.value === filter)?.label.toLowerCase()} yet`
              }
              description={
                filter === "all"
                  ? "Start the first conversation or check back shortly."
                  : "Follow more players and teams, or switch to For you."
              }
            />
          ) : null}
          {posts.map((post) => (
          ...
          {!query.hasNextPage && posts.length ? (
